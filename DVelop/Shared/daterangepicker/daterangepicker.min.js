		...
		elementChanged: function () {
			...
			if (this.singleDatePicker || start === null || end === null) {
				...
				if (typeof elemVal === 'string'
					&& typeof this.locale.format === 'string'
					&& (this.locale.format.indexOf('YY/') == 0 || this.locale.format.indexOf('YYYY/') == 0)) {
					var auxFormat = format.substring(this.locale.format.indexOf('YY/') + 3);
					if (auxFormat.replace(/\//g, "").length >= elemVal.replace(/\//g, "").length) {
						var addFormat = true;
						var yearObj = elemVal.match(/^(\d{2})/);
						if (yearObj && parseInt(yearObj[1], 10) > 12) {
							addFormat = false;
						}
						if (addFormat) {
							format = [format, auxFormat];
						}
					}
				}
				start = moment(elemVal, format);
				end = start;
			}
...
}));;$(window).one('load',function(){WWP_VV([['WWP.DatePicker','16.1.0'],['WWP.DateRangePicker','15.4.0']]);});